...
quant_cutoff = 0.95; % observed original correlation should be greater than this quartile
min_edges = 0.05;
...
no_cal_corr_event = 3;
ijlag_rows = 3;
min_adf_length = 50; % min length of data for adf test.

no_permutations = 10000; % no.of permutations to calculate p_value
% check if the time-series is stationary

%% loading all .mat files and calculating network
...
% calculate network for each phase
...
for cal_corr_event = 2:2%no_cal_corr_event
    ...
    for e = 1:no_exp
        ...
        for iter = 1:no_it
            ...
            all_not_cross_id = isnan(en_end(:,iter,e));
            if sum(all_not_cross_id) > 0
                continue
            end
            ...
            for i = 1:n
                ...
                for f_pt = 1:no_it
                    ...
                    if f_pt ~= iter
                        ...
                        all_not_cross_id = isnan(en_end(:,f_pt,e));
                        if sum(all_not_cross_id) > 0
                            continue
                        end
        ...
        % constructing network using simple permutation

        cc_nan_id = abs(cc_pt_trl_nnan_all(:,3)) > min_lag;
        cc_pt_trl_nnan_all = cc_pt_trl_nnan_all(cc_nan_id,:);

        eff_no_it_id = find(isnan(cc_pt_trl_nnan_all(1,:)));
        eff_no_it = no_it - length(eff_no_it_id);
        cc_pt_trl_nnan_all = cc_pt_trl_nnan_all(:,1:(eff_no_it+ijlag_rows));
        ...
        for l = 1:n
            ...
            for f = 1:n
                ...
                if l ~= f
                    ...
                    avg_edges_lf = (no_l_lead)/eff_no_it;
            ...
            for j = 1:n
                ...
                if j ~= i
                    ...
                    for p = 1:no_permutations
                        ...
                        pert_ord = randperm(eff_no_it, size(cc_pair_ij,1)) + (col_cc_pair-1);
    ...
    % avg_combi_net = [l, f, frac_lead, avg_cc]
...
% writelines(end_statement, strcat('end_statement_pm_', fname, '.txt'))
